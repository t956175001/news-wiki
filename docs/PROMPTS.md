...
> 所有 prompt 的**全文**在此。实现时把这些文本作为 `PromptVersion` 的初始 v1 灌进数据库（`apps/common/prompts/seeds.py`），代码里只按 `key` 取用，**不要在 Python 里硬编码 prompt 字符串**。

## 通用约定

- 模板变量用 Python `str.format` 风格的 `{var}` 占位，由 `apps/common/prompts/service.py::render(key, ctx)` 渲染
- 所有抽取类调用带 `response_format={"type": "json_object"}`
- 所有输出必须是**单个 JSON 对象**，根键固定
- 每个抽取项必须带 `raw_article_id`，值来自输入语料中的 `ARTICLE_ID` 标记——**这是证据能挂回原文的唯一依据**
- 每个抽取项必须带 `evidence`，为**原文中的连续片段**，不得改写、不得拼接、不得超过 200 字
- `confidence` 为 0.0-1.0 的小数

## 语料格式

`apps/wiki/services/extract_pipeline.py::build_corpus(articles)` 产出：
...
## `wiki.extract_entities` (v1)

**用途**：第一步，从语料中抽出具体实体。

```
你是一个信息抽取引擎。从下面的资讯语料中抽取出所有**具体实体**。
...
## `wiki.extract_concepts` (v1)

**用途**：第二步，抽取抽象概念。入参带上第一步的结果，避免与实体重复。

```
你是一个信息抽取引擎。从下面的资讯语料中抽取出**抽象概念**。
...
**变量**：`raw_text`, `entities_json`
**输出根键**：`concepts`

---

## `wiki.extract_linkages` (v1)

**用途**：第三步，抽取实体之间、实体与概念之间的关系。这是图谱有没有边的关键。

```
你是一个信息抽取引擎。从下面的资讯语料中抽取**关系三元组**。
...
**变量**：`raw_text`, `entities_json`, `concepts_json`
**输出根键**：`linkages`
...
```
你是一名 AI 领域的资讯编辑。根据下面的素材，写一份当日 AI 资讯简报。
...
**变量**：`date`, `articles_json`, `entities_json`, `linkages_json`
**输出根键**：无（顶层就是对象，含 `title` / `content_md` / `used_indexes`）

> `articles_json` 的每项形如 `{"index":1,"id":42,"title":"...","url":"...","publish_time":"..."}`。
> 生成后由 `apps/brief/services/generate.py` 按 `used_indexes` 反查构造 `DailyBrief.citations`，**不信任 LLM 自己输出的引用元数据**。

---

## 校验规则

`apps/wiki/services/validators.py` 对每个 schema 做校验，失败抛 `SchemaError`（触发 tenacity 重试）：

| 检查 | 处理 |
|---|---|
| 根键缺失或非数组 | 抛 `SchemaError` |
| 单项非 dict | 抛 `SchemaError` |
| 必填字段缺失（`name`/`type`/`raw_article_id` 等） | 抛 `SchemaError` |
| `confidence` 非数字 | 抛 `SchemaError`；能转数字则 clamp 到 [0,1] |
| `aliases` / `signals` 非 `list[str]` | 抛 `SchemaError` |
| `raw_article_id` 不在本批次 ID 集合中 | **跳过该项**，计入 `skipped_invalid_article_ids`，不抛错 |
| `evidence` 为空 | 保留该项但不建 Evidence 记录 |
| `evidence` 不是原文子串 | **降级处理**：保留，但 `confidence *= 0.8` 并在 run 日志里计数。不抛错（LLM 常有全角/半角、空白差异） |
| `linkage.subject`/`object` 不在已抽取列表中 | **跳过该项**，计入 `skipped_unknown_refs` |
| 结果为空数组 | 合法，不重试 |

> `evidence` 子串检查前先做归一：去除所有空白字符再比对。
...
| Prompt Key | 版本 | 日期 | 变更 |
|---|---|---|---|
| `wiki.extract_entities` | v1 | 2026-08-28 | 初版 |
| `wiki.extract_concepts` | v1 | 2026-08-28 | 初版 |
| `wiki.extract_linkages` | v1 | 2026-08-28 | 初版 |
| `brief.daily` | v1 | 2026-08-28 | 初版 |

> **调 prompt 时的纪律**：改动必须新建一个 `PromptVersion`，不要就地改 v1 的文本。这样词条页上"本条由 v1 抽出"才是真的，也才有东西可以在面试里讲「我怎么迭代 prompt」。每次新版在上表追加一行，写清改了什么、为什么。
