...
### 让数据随时间收敛（ADR-019 / ADR-020）

上线两周后线上是 313 篇文章 → 1705 个节点（约 9.1 个/篇），92% 的实体只被提到过一次，
度数 ≥1 的节点已超过 API 硬上限 500，而画布只画 100 个。
根因不是「节点数设多了」，是**图谱没有时间维度**：一张只涨不收的全时累积图，
透过固定大小的取景框看，越往后越是同一批巨型枢纽加一个越来越任意的切片。

- **图谱加时间窗**（`days`，默认 30 天，传 0 取全部）。按**文章发布时间**算而不是
  `Linkage.created_at`——积压是倒序消化的，用抽取时间会把两个月前的旧闻算成今天的新闻。
  日期不可知的关系一律保留：窗口的职责是让旧新闻退场，不是把无法判断年龄的东西藏掉。
- **默认隐藏 `涉及` 这个谓词**（可在页面上取消）。它在线上占全部边的 **17%**，
  而「A 涉及 B」是关系的缺席而非一种关系。59 个谓词里 27 个只出现一次。
- **节点度数改为按可见的边计算**。这是前两条的前提：过滤掉边却仍用库里的总度数，
  会画出又大又没有连线的节点，Top-N 也会按不存在的边排序。
- **实体身份改为名字本身**：`normalize_name` 去掉全部分隔符，唯一键不再包含 `entity_type`。
  线上 19 组重名逐条核过，**每组都是同一个真实主体被标了不同类型**
  （GitHub、Hugging Face、DeepSeek、Twitter、Facebook、Gemini、LinkedIn……），
  没有一组是真正的同名异物——原先「Claude 模型和 Claude 产品是两回事」的假设方向反了，
  实际的失败模式是同一个东西被类型标签劈成两半，把它的关系也劈成两半。
  迁移在真实数据副本上演练过：**1049 条证据、452 条关系一条没丢**，无悬空外键；
  线上 dry-run 预测合并 19 行（1299 → 1280）。
- **陈旧的待抽取文章直接退休**。线上 126/313 篇从未被抽取——采集速度结构性高于抽取上限，
...
- **「流水线」改称「工作流」**。侧边栏、页头、页面标题、证据卡的跳转提示与 API 文档一并改；
  `CHANGELOG` / `ROADMAP` / `CLAUDE.md` 里的历史记录保留原词，它们记的是当时的事。

- **图谱默认节点上限从 150 降到 100**（前后端一起改，见 ADR-018）。150 个节点在
  1600×900 的画布上互相压着、标签重叠、底部和右侧还溢出画布，读不出结构；
  100 个节点各簇是分开的，枢纽和它的叶子看得清。
  代价是同一份演示数据下可见的边从 163 条降到 95 条（少 42%）——看到的关系变少，
  但看得清的变多，想看全的把滑块拉回去即可（滑块上限仍是 300，API 硬上限仍是 500）。
  `ARCHITECTURE.md` §4.2 和端点表里的默认值一并订正。
- **图谱里点节点改成聚焦式：点击＝把焦点移到该节点，点已经是中心的那个节点＝进词条**。
  此前在全图上点一个实体就直接跳走，等于让人在「想看看这个点连着谁」和「离开这一页」之间
  没有中间档；而看邻居恰恰是图谱页唯一的存在理由，绕一圈还得从词条页点「在图谱中查看」再回来。
  顺带消掉一个旧的不一致：实体点了跳走、概念点了换中心，同一个手势两种结果，
  而访客在点下去之前无从分辨节点是哪一类。现在规则只有一条，且在全图和 ego 图里完全相同——
  这是先做了一版「ego 图里点任意节点都进词条」之后改的，那一版试用下来 ego 图是单跳死胡同：
  点邻居就离开了，没法顺着关系链继续走。
  中心节点每次重新布局都会换位置，所以横幅上另放一个「查看词条 →」兜底。
