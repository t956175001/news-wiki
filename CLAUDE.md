...
- **2026-08-30**：D12 完成，**已上线：https://newswiki.cn**（www 同源别名）。实际资源与 `DEPLOYMENT.md` 原始设定有出入：**阿里云轻量应用服务器，首尔**（不是腾讯云香港），域名 **newswiki.cn**（不是 .app/.dev/.com）。首尔没有 HK 对大陆的线路优势，所以 ADR-006 的"直连优于 CDN"前提这次是真要靠实测验证，不是走过场。
  ...
  **待办**：手机 4G 打开首页 3 秒出内容——留给用户自己确认，我这边测不了。
  下一步执行 `docs/ROADMAP.md` 的 D13（README + 仓库门面）。
- **2026-08-30**：D13 完成。README.md 从占位骨架填成完整门面：顶部一句话定位 + 在线地址 + 演示 GIF、四条核心特性各配一张真实截图、`docs/ARCHITECTURE.md` §2 的 mermaid 数据流图、技术栈表、三行 `docker compose` 快速开始、从 14 条 ADR 里展开写的「设计取舍」（Playwright/三步抽取/Evidence 三可空外键/不用 Celery/成本护栏 各一节）、项目结构树、测试与 CI 说明、License。
  ...
  **项目话术稿**（本地工作笔记，2026-09-02 起不入仓库）补完全部 `<X>/<Y>/<Z>/<N>` 占位符时定了一条纪律：不编"抽取成功率从 X% 提升到 Y%"这类虚构对比，一律换成线上真实数字——11 次运行的实际分布（10 成功/1 部分成功/0 完全失败）、`LLM_DAILY_BUDGET_CNY` 的实际配置值 ¥5（并注明日均花费远低于阈值）、词条详情从"压到 <10 条查询"改成精确的"固定 5 次"（ADR-003 的真实断言值）、真实的 GitHub/演示链接、ADR 计数从"12 条"更正为实际的 14 条。
...
- **2026-08-30**：D14 完成（14 天路线图全部执行完毕）。全站走查用 Playwright（Chrome 扩展本 session 仍未连接）+ 本机 sqlite 环境完成，过程中发现并修了三个真 bug，其中一个和"性能分"直接相关、一个和"线上到底跑的是哪版代码"直接相关：
  **① 移动端布局整个不可用**：`AppSider`/`DefaultLayout` 从来没写过响应式样式，375px 视口下固定 232px 侧边栏几乎吃光屏幕，正文被挤成一字一行。改成窄屏下侧边栏转水平顶栏，过程中牵出**第二个更隐蔽的 bug**：`.app-shell` 转 column 方向后，`.app-shell__body{flex:1}` 缺一个配套的 `min-height:0`（桌面端靠 `min-width:0` 处理的是同一件事在 row 方向的版本），导致它撑到内容真实高度而不是被 `overflow:hidden` 的外层截断——超出首屏的内容不是能滚动，是彻底不可达。两个 `min-*:0` 必须成对出现，换轴向时最容易只改一半。
  **② 首屏主 bundle 1.48MB（gzip 458KB）**：`main.ts` 一直 `app.use(Antd)` 全量注册 Ant Design Vue，`GraphChart.vue` 也是 `import * as echarts from 'echarts'` 整包引入（D8 就记在 BACKLOG 里但没处理）。换成 `unplugin-vue-components`（`AntDesignVueResolver`）按需解析 + `echarts/core` 按需注册，主 chunk 降到 116KB（gzip 45KB，**降 92%**）。
  **③ 部署链路里最大的一个坑，是验证②的效果时才炸出来的**：本地反复确认 Lighthouse 分数从 0.55 升到 0.86 后推到线上验证，结果生产环境纹丝不动，还在吃 1.48MB 的旧 bundle——排查发现 `docker-compose.prod.yml` 里 `frontend_dist` 是个**具名卷**，Docker 只在卷第一次被创建时拿镜像同路径内容做种，此后每次 `--build` 都在正确构建新镜像，可运行时这个路径全被那份"卷出生时"的旧内容盖住，Caddy 读的又正好是这个卷——等于**从这个卷诞生的那一刻起，所有后续的前端改动理论上都没有真正上线过**，只是每次部署报"成功"掩盖了这一点。修法：镜像里把前端产物构建到独立路径 `frontend_dist_build`，`web` 容器每次启动时用 `command` 里新增的 `rm -rf ... && cp -r ...` 把它复制进卷挂载的 `frontend_dist`，让卷内容永远跟着"这次到底部署的是哪个镜像"走，而不是"卷出生那次部署的是哪个镜像"。`docs/DEPLOYMENT.md` 里描述这个机制的说明和 Dockerfile 示例一并订正，不然文档教出来的还是错的做法。
  **Lighthouse 复测的诚实记录**：本机是非受控环境（Lighthouse 官方文档本身就提示这类环境结果会抖动），生产站点连续实测 5 次：0.75/0.67/0.83/0.66/0.76——FCP/LCP/TBT/CLS 这四项核心指标每次都稳定落在"good"区间（FCP 1.8-3.5s、LCP 3.7-5.2s、TBT ~250-290ms、CLS ~0.001-0.002），真正来回摆的只有 Speed Index 单项（2.6s-9.6s），和本机后台进程/CPU 调度状态强相关，和站点本身变没变快关系不大。没有挑一次最高分说"达标"，如实记录这个区间——bundle 从 1.48MB 降到 116KB 是确定的、可复现的事实，综合分数因本机测量噪声在 0.66-0.87 间浮动是另一个独立的事实，两者都要说清楚。
  **加载/空/错误态走查**：用 `page.route` 分别模拟延迟（验证骨架屏而非白屏）、`route.abort('failed')`（验证 `ErrorState` 渲染"Network Error"+ 重试按钮而非白屏）、访问零关系词条（`Aakriti Shah`，验证空态文案）、访问不存在 ID（验证"该实体不存在"），全部有内容可看，没有白屏。**发现一个不影响判据但记进 BACKLOG 的小瑕疵**：网络层面真失败时 `errorMessage` 直接显示 `e.message`（浏览器原生英文错误），和页面其余中文文案不一致。
  **Slow 3G 判据**：用 CDP `Network.emulateNetworkConditions` 模拟 Chrome DevTools 的 Slow 3G 预设（400ms 延迟、500kbps 上下行），首页在约 1.2 秒时已经画出侧边栏和头部（非空白），2.5 秒左右整页加载完成。
  **D1-D13 判据重跑**：backend `pytest -q` 488 个全过、`ruff check`/`ruff format --check` 全绿；frontend `npm run test`（39 个）/`lint`/`vue-tsc --noEmit`/`build` 全绿；`scripts/check-clean.sh` 全绿；线上关键端点（health/OpenAPI docs/cron 无 token 403/brief/graph/ops stats）与五个前端路由全部 200。**没有逐条重放 D1-D12 每一天当时写的手工 curl 判据**（很多依赖当时本机没有的 Docker/Postgres 或早已被后续实现取代），按"这天的验收精神"重跑等价的自动化判据，比逐字重放旧命令更能反映当前真实状态。
  **30 秒演示视频**：本机没有可用的屏幕录制工具，做法和 D13 的 GIF 一脉相承但升级成视频——6 张 Playwright 截图（首页简报 → COVID-19 词条证据展开 → Prompt 版本模板 → 抽取流水线分步时间线 → 运行聚合统计 → 关系图谱）配中文字幕（`msyh.ttc` 用 Pillow 画字幕条），imageio + 一个装进本机 venv 的静态 ffmpeg 二进制（`imageio[ffmpeg]`，同样没进 `requirements.txt`）编码成 30.0 秒、1280x720、H.264 的 `docs/images/demo-video.mp4`（711KB）。三段字幕分别对应"它做什么/证据溯源/AI 工程"，AI 工程那段直接用 Prompt 面板和流水线时间线的真实截图，不是复述。
